...
let menuTotal = [latte, burger, breakfastBurrito]
...
const discountTable = [
  ...
  {type: "student" , cost: 0.90 },
  {type: "public"  , cost: 1.00 }
]

function burgerDiscount(myRole){
  for (let i = 0; i < discountTable.length; i++){
    if (discountTable[i].type === myRole){
      let newPrice = burger.price * discountTable[i].cost;
      return newPrice;
    }
  }
}
...
reviews.push({name: "Seth", rating: 5, feedback: "Best place I would never come back to"});
...
//sample code from Nathan's codepen
